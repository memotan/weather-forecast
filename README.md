...
対象地域: 中野区・墨田区（東京）。`weather_notify/config.py` の `LOCATIONS` で変更できます。

## 通知の見方

1地域につき1行で表示されます。例: `中野区  ☀️晴れ🟢  ☔20%  🌡29/21℃`
...
GitHub Actions の `schedule` (cron, UTC) で3回/日実行します。毎時0分・30分はActionsの負荷が
集中し実行が遅れやすいため（GitHub公式の既知の制約）、意図的に数分ずらしています。

| JST(目安) | UTC (cron)    | 内容                     |
| ---------- | ------------- | ------------------------ |
| 7:37       | `37 22 * * *` | 本日1日分の予報           |
| 12:07      | `7 3 * * *`   | 本日午後〜夜（12時以降）の予報 |
| 20:07      | `7 11 * * *`  | 翌日1日分の予報           |

それでも数分〜十数分程度の前後は起こり得ます（GitHub Actions側の仕様）。
